...
import {
  Search, ChevronDown, Plus, Calendar, Edit2, X,
  RefreshCw, Clock, User, Check
} from 'lucide-react';
...
const initialAppointments: Appointment[] = [
  { id: 1, patient: 'John Doe', doctor: 'Dr. Smith', date: new Date(2026, 2, 24), time: '10:00 AM', status: 'Confirmed', avatar: '👨', notes: 'Regular checkup' },
  { id: 2, patient: 'Jane Smith', doctor: 'Dr. Lee', date: new Date(2026, 2, 24), time: '11:30 AM', status: 'Pending', avatar: '👩', notes: 'Follow-up consultation' },
  { id: 3, patient: 'Mike Johnson', doctor: 'Dr. Williams', date: new Date(2026, 2, 24), time: '02:00 PM', status: 'Confirmed', avatar: '👨' },
  { id: 4, patient: 'Sarah Williams', doctor: 'Dr. Brown', date: new Date(2026, 2, 24), time: '03:30 PM', status: 'Cancelled', avatar: '👩' },
  { id: 5, patient: 'Robert Chen', doctor: 'Dr. Smith', date: new Date(2026, 2, 25), time: '09:00 AM', status: 'Pending', avatar: '👨' },
  { id: 6, patient: 'Emily Davis', doctor: 'Dr. Lee', date: new Date(2026, 2, 25), time: '01:00 PM', status: 'Confirmed', avatar: '👩' },
];
...
const BookModal: React.FC<BookModalProps> = ({ onClose, onSave, editData, rescheduleMode }) => {
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <div>
            <h2 className="modal-title">
              {rescheduleMode ? 'Reschedule Appointment' : editData ? 'Edit Appointment' : 'Book New Appointment'}
            </h2>
            <p className="modal-subtitle">
              {rescheduleMode ? 'Pick a new date and time for this appointment' : editData ? 'Update appointment details' : 'Fill in the details to schedule an appointment'}
            </p>
          </div>
        ...
        <div className="modal-body">
          ...
          <div className="modal-field">
            <label className="modal-label">Patient Name <span className="modal-required">*</span></label>
            <input
            ...
            />
            {errors.patient && <span className="modal-error">{errors.patient}</span>}
          </div>
          ...
          <div className="modal-field">
            ...
            <div className="modal-select-wrap">
              <select className="modal-select" value={form.doctor} onChange={e => set('doctor', e.target.value)}>
          ...
          {/* Date & Time */}
            ...
            <div className="modal-field">
              ...
              <div className="modal-select-wrap">
                <select className="modal-select" value={form.time} onChange={e => set('time', e.target.value)}>
        ...
        <div className="modal-footer">
          ...
          <button className="modal-btn-save" onClick={handleSubmit}>
            {rescheduleMode ? 'Confirm Reschedule' : editData ? 'Save Changes' : 'Book Appointment'}
          </button>
        </div>
      </div>
    </div>
...
const ConfirmModal: React.FC<ConfirmModalProps> = ({ name, onConfirm, onCancel }) => (
  <div className="modal-overlay" onClick={onCancel}>
    <div className="modal-box modal-box--sm" onClick={e => e.stopPropagation()}>
      ...
      <div className="modal-body">
        <p className="confirm-text">Are you sure you want to cancel <strong>{name}</strong>'s appointment? This action cannot be undone.</p>
      </div>
      <div className="modal-footer">
        <button className="modal-btn-cancel" onClick={onCancel}>Keep Appointment</button>
        <button className="modal-btn-danger" onClick={onConfirm}>Yes, Cancel It</button>
      </div>
    </div>
  </div>
);
...
const Appointments: React.FC = () => {
  ...
  const [confirmCancel, setConfirmCancel] = React.useState<Appointment | null>(null);
  ...
  React.useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (doctorRef.current && !doctorRef.current.contains(e.target as Node)) setShowDoctorDropdown(false);
    };
...
const filtered = appointments.filter(a => {
  ...
  const handleSave = (appt: Appointment) => {
    if (editData) {
      ...
      setSelected(appt);
      showToast(rescheduleMode ? 'Appointment rescheduled successfully' : 'Appointment updated successfully');
    } else {
  ...
  const handleCancelConfirm = () => {
    ...
    setAppointments(prev => prev.map(a => a.id === confirmCancel.id ? updated : a));
    setSelected(updated);
  ...
  return (
    <div className="appt-container">
      ...
      <div className="appt-toolbar">
        <div />
        <button className="appt-book-btn" onClick={() => { setEditData(null); setRescheduleMode(false); setShowBookModal(true); }}>
...
<div className="appt-cal-nav">
  ...
  <button className="appt-cal-nav-btn" onClick={handlePrevMonth}>&lt;</button>
  ...
  {/* Next Month Button */}
  <button className="appt-cal-nav-btn" onClick={handleNextMonth}>&gt;</button>
            </div>
          ...
          {selected ? (
            <>
              <div className="appt-detail-top">
                ...
                <span className={`appt-badge ${selected.status.toLowerCase()}`}>{selected.status}</span>
              </div>
              ...
              <div className="appt-detail-rows">
                <div className="appt-detail-row">
                  ...
                  <span className="appt-detail-row-value">{selected.doctor}</span>
                </div>
                <div className="appt-detail-row">
                  <Calendar size={14} />
                  <span className="appt-detail-row-label">Date</span>
                  <span className="appt-detail-row-value">{selected.date.toDateString()}</span>
                </div>
                <div className="appt-detail-row">
                  <Clock size={14} />
              ...
              <div className="appt-detail-actions">
                <button className="appt-action-btn edit" onClick={() => handleEdit(selected)}>
                  <Edit2 size={14} /> Edit
                </button>
                <button className="appt-action-btn reschedule" onClick={() => handleReschedule(selected)}>
                  <RefreshCw size={14} /> Reschedule
                </button>
                <button className="appt-action-btn cancel" onClick={() => setConfirmCancel(selected)}>
                  <X size={14} /> Cancel
                </button>
              </div>
            </>
          ...
          <div className="appt-table-filters">
            ...
            <div className="appt-filter-wrap" ref={dateRef}>
              <div
                ...
                onClick={() => { setShowDateDropdown(p => !p); setShowDoctorDropdown(false); }}
              >
                <Calendar size={14} />
            ...
            <div className="appt-filter-wrap" ref={doctorRef}>
              <div
                ...
                onClick={() => { setShowDoctorDropdown(p => !p); setShowDateDropdown(false); }}
              >
                <User size={14} />
                {doctorFilter === 'All' ? 'Doctor' : doctorFilter}
                <ChevronDown size={14} className={showDoctorDropdown ? 'rotated' : ''} />
              </div>
              {showDoctorDropdown && (
                <div className="appt-dropdown appt-dropdown--right">
                  {DOCTOR_OPTIONS.map(opt => (
                    <div
                      ...
                      onClick={() => { setDoctorFilter(opt); setShowDoctorDropdown(false); }}
                    >
        ...
        <div className="appt-table-wrapper">
          <table className="appt-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="appt-table-empty">No appointments found</td>
                </tr>
              ) : filtered.map(apt => (
                <tr key={apt.id} className={`appt-table-row ${selected?.id === apt.id ? 'row-selected' : ''}`} onClick={() => setSelected(apt)}>
                  <td>
                  ...
                  <td>
                    <div className="appt-time-cell"><Clock size={13} /> {apt.time}</div>
                  </td>
                  <td>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <div className="appt-actions-cell">
                      <button className="appt-icon-btn" title="Edit" onClick={() => handleEdit(apt)}><Edit2 size={14} /></button>
                      <button className="appt-icon-btn" title="Reschedule" onClick={() => handleReschedule(apt)}><RefreshCw size={14} /></button>
                      <button className="appt-icon-btn danger" title="Cancel" onClick={() => setConfirmCancel(apt)}><X size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showBookModal && (
        <BookModal
          onClose={() => { setShowBookModal(false); setEditData(null); setRescheduleMode(false); }}
          onSave={handleSave}
          editData={editData}
